...
| `RENEWAL_OVERDUE` | Tenure has expired (`today > tenure_end_date`) and the customer has not renewed (~4%). Item is still physically with the customer. All VAS expire on first entry. A monthly cycle runs: `renewal_overdue_cycle_start_date` and `renewal_overdue_cycle_end_date` advance by one month on each cycle, `months_since_renewal_overdue` increments, and `currently_npa` flips to `true` once overdue months exceed the NPA threshold. Only applies to `FURLENCO_RENTAL` vertical — UNLMTD items skip this state (handled at Plan level). Autopay-enabled items skip RENEWAL_OVERDUE entirely and go straight to auto-renewal. |
...
| Renewal-eligible (UNLMTD swap items only) | `STATES_ELIGIBLE_FOR_RENEWAL_WITH_SWAP` | `AWAITING_STOCK`, `DELIVERY_TO_BE_SCHEDULED`, `DELIVERY_SCHEDULED`, `DELIVERY_IN_TRANSIT`, `OUT_FOR_DELIVERY`, `DELIVERED`, `ACTIVE`, `PICKUP_TO_BE_SCHEDULED`, `PICKUP_SCHEDULED`, `OUT_FOR_PICKUP` |
| Rent-to-purchase eligible | `RTO_ELIGIBLE_STATES` | `ACTIVE`, `RENEWAL_OVERDUE` |
| Purchasable | `PURCHASABLE_STATES` | `ACTIVE`, `PICKUP_TO_BE_SCHEDULED`, `PICKUP_SCHEDULED`, `RENEWAL_OVERDUE` — **rental verticals only** (`FURLENCO_RENTAL`, `UNLMTD`); `isPurchasable()` additionally guards on `vertical.isRentalVertical()` |
