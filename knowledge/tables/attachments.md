...
| `RENEWAL_OVERDUE` | Tenure has expired (`today > tenure_end_date`) and the customer has not renewed (~4%). Attachment is still physically with the customer. All VAS expire on first entry. A monthly cycle runs: `renewal_overdue_cycle_start_date` and `renewal_overdue_cycle_end_date` advance by one month on each cycle, `months_since_renewal_overdue` increments, and `currently_npa` flips to `true` once overdue months exceed the NPA threshold. Only applies to `FURLENCO_RENTAL` vertical — UNLMTD attachments skip this state (handled at Plan level). Autopay-enabled attachments skip RENEWAL_OVERDUE entirely and go straight to auto-renewal (autopay is inherited from the parent composite_item). |
